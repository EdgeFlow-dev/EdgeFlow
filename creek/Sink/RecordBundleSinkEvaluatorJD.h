 ...
 *
 * Replica of RecordBitmapBundleSinkEvaluator1.h
 */

#ifndef RECORDBUNDLESINKEVALUATORJD_H_
#define RECORDBUNDLESINKEVALUATORJD_H_
...
class RecordBundleSinkEvaluatorJD: public TransformEvaulator<RecordBundleSink<InputT>> {
	using TransformT = RecordBundleSink<InputT>;
//	using InputBundleT = RecordBundle<pair<long,vector<long>>>;
//	using OutputBundleT = RecordBundle<pair<long,vector<long>>>;
	using InputBundleT = RecordBundle<InputT>;
	using OutputBundleT = RecordBundle<InputT>;

public:
	RecordBundleSinkEvaluatorJD(int node){
	...
	bool process_bundle(TransformT* trans, EvaluationBundleContext* c,
		...
		for (auto it = input_bundle->begin(); it != input_bundle->end(); ++it) {
			/* according to hym, this is for testing purpose only. by design,
			 * a sink should drop all data bundles
			 */
			output_bundle->add_record(Record<InputT>((*it).data, (*it).ts));
		}
...
#endif /* RECORDBUNDLESINKEVALUATORJD_H_ */
