 ...
 *      Author: xzl
 *
 * FixedWindowInto + Mapper batched together, since they'll touch each record
 *
 * always produce WindowsBundle
 *
 * Modeled after GrepMapper.h
 */
...
//	}
  ...
  uint64_t do_map(Record<InputT> const & in,
    ...
    long offset = (in.ts - this->start) % (this->window_size);

    output_bundle->add_value(
          Window(in.ts - offset, this->window_size),
