...
public:
	...
	UnboundedInMemEvaluator_Join(int node) : current_ts(0) { }
	...
	void evaluate(TransformT* t, EvaluationBundleContext *c,
		...
		etime delta = 1000 * t->punc_interval_ms / bundle_per_interval;
						...
						bundle->add_record(
							...
							Record<pair<long, long>>(
								std::pair<long, long>(
									...
									oid //set to 0 or 1   --> xzl: can't we use real values?
								),
								current_ts + delta * i
							)//Record(data, ts)
						);//add_record
