...
#ifndef CREEK_UNBOUNDEDSEQZMQEVAL_H
#define CREEK_UNBOUNDEDSEQZMQEVAL_H
...
class SourceSeqZmqEval
		: public UnboundedInMemEvaluatorBase<T, RecordSeqBundle, SourceSeqZmq<T>> {
...
//	using TransformT = typename UnboundedInMemEvaluatorBase<T, RecordSeqBundle>::TransformT;
	using TransformT = SourceSeqZmq<T>;
	using BaseT = UnboundedInMemEvaluatorBase<T, RecordSeqBundle, SourceSeqZmq<T>>;

	using TicketT = std::future<void>;

public:

	SourceSeqZmqEval(int node) { }
...
#else
			zmq_s.rbegin()->setsockopt(ZMQ_SNDHWM, &hwm, sizeof(hwm));
#endif
			zmq_s.rbegin()->connect(t->input_socket);
		}
...
//#if defined(USE_TZ)
			...
			for (int task_id = 0; task_id < total_tasks; task_id++) {
				...
				auto source_task_lambda = [t, &total_tasks,
						this, &delta, &out, &c, &records_per_bundle, &num_nodes, task_id, &offset, &zmq_s](int id) {
						auto range = get_range(config.bundles_per_epoch, total_tasks, task_id);

						/* each task produces bundles in the range to each output  (NOT in round robin */
						for (auto oid = 0u; oid < t->num_outputs; oid++) {
							auto local_offset = (offset[oid] + records_per_bundle * range.first) % t->buffer_size_records;

							auto bundles_per_out = range.second / t->num_outputs;
							...
							/* for i-th output, we only produce a fraction of bundles */
							for (unsigned i = range.first + oid * bundles_per_out;
									 i < range.first + oid * bundles_per_out + bundles_per_out; i++) {
								/* construct the bundles by reading NUMA buffers round-robin */
								int nodeid = (i % num_nodes);

								/* grab a bundle from network */
								zmq::message_t msg;
								zmq_s[task_id].recv(&msg);

								/* sanity check */
								xzl_bug_on_msg(msg.size() % sizeof(Record<T>) != 0, "misaligned record boundary. check # items per record?");
								if (msg.size() / sizeof(Record<T>) != records_per_bundle) {
									EE("bug? msg has %lu records, which mismathces local records_per_bundle %lu",
										 msg.size() / sizeof(Record<T>), records_per_bundle);
									xzl_bug("");
								}
...
//			t->byte_counter_.fetch_add(t->records_per_interval * sizeof(Record<T>) * t->num_outputs,
//																 std::memory_order_relaxed);
//			t->record_counter_.fetch_add(t->records_per_interval * t->num_outputs,
//																	 std::memory_order_relaxed);

			/* in this round, we indeed issue # bundles == config.bundles_per_epoch, regardless of num_outputs */
			t->byte_counter_.fetch_add(config.bundles_per_epoch * records_per_bundle * sizeof(Record<T>),
																 std::memory_order_relaxed);
			t->record_counter_.fetch_add(config.bundles_per_epoch * records_per_bundle,
																	 std::memory_order_relaxed);
...
#endif //CREEK_UNBOUNDEDSEQZMQEVAL_H
