...
template<typename T>
class SourceSeq : public PTransform {
...
public:
	...
	SourceSeq (string name, const char *input_fname,
...
//		struct stat finfo;
		...
		unsigned long expected_record_num = records_per_interval * 2 * 2;
//		int num_nodes = 1;
//		std::cout << "WARNING: num_nodes is set to 1!!!!! Reset it in Unbounded.h and SourceSeqEvaluator.h!!!" << std::endl;
		...
		printf("source file: %s (specified as: %s)\n", rpath, input_fname);
		free(rpath);
		printf("buffer size: %.2f MB\t", (double)raw_buf_size/1024/1024);
		printf("# of Records: %ld \n", buffer_size_records);
		...
		for(int i = 0; i < num_nodes; i++) {
			...
			buffer_size_records = ParseCSVFile(input_fname, p, raw_buf_size / sizeof(T));
...
template<>
unsigned long
SourceSeq<creek::kvpair>::ParseCSVFile(const string & fname, creek::kvpair* p, unsigned long len);
