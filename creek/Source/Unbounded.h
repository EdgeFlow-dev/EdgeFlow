...
public:
  ...
  uint64_t record_len; /* the length covered by each record */
...
public:
      ...
      byte_counter_(0), record_counter_(0)
  {
    ...
    buffer_size = std::max(CONFIG_MIN_PERNODE_BUFFER_SIZE_MB * 1024 *1024,
    											 CONFIG_MIN_EPOCHS_PERNODE_BUFFER * records_per_interval * record_len);
...
//					printf("read one buffer. %.2f \n", (double)r/buffer_size);
    ...
    for (int i = 0; i < num_nodes; i++) {
    	...
    	for (unsigned int j = 0; j < buffer_size_records; j++) {
    		record_buffer[j].data.data = buffers[i] + j * record_len;
    		record_buffer[j].data.len = record_len;
...
//        buffers.size(), buffer_size_records, buffer_size/1024/1024, input_fname);
  		...
  		uint64_t record_size, int session_gap_ms)
  {
		...
		xzl_assert(record_len >= 140); /* must have enough data */
...
//						std::min(text_len, buffer_size - r), /* max to copy */
    		...
    		uint64_t record_size, int session_gap_ms)
  {
		...
		xzl_assert(record_len >= 200); /* must have enough data */
...
/* Used to be for Join. Now also for test-winsum-long
 *
 * xzl: change input file format:
 *
 * a text file.
 * line 1: total # of ints in the file
 * line 2+: each line is an int.
 *
 * see test/digit-gen.py
 *
 * For Join's source, see UnboundedInMem_Join.
 *
 * */
template<template<class> class BundleT>
class UnboundedInMem<long, BundleT> : public PTransform {
...
public:
  ...
  UnboundedInMem (string name, const char *input_fname,
	...
	//	int num_nodes = numa_num_configured_nodes();
		...
		xzl_bug_on(record_len != sizeof(long));
		...
		/* 1st line of the input file is # of ints */
		fscanf(file, "%ld\n", &record_num);
