...
public:
	...
	virtual creek::etime getCurrentTimestamp() = 0;
...
public:
	 ...
	 */
	creek::etime getCurrentTimestamp() {
...
public:
	...
	long getEstimatedSizeBytes(PipelineOptions* options) {
		long trueEndOffset =
				(_endOffset == etime_max) ? getMaxEndOffset(options) : _endOffset;
...
public:
	...
	OffsetBasedSource<T>* splitAtFraction(double fraction) {
		...
		if (_rangeTracker.getStopPosition() == etime_max) {
			printf(
