...
//template<class T, template<class> class BundleT, class TransformT_ = UnboundedInMem<T, BundleT>>
template<class T, template<class> class BundleT, class TransformT_>
class UnboundedInMemEvaluatorBase
//		: public TransformEvaulator<UnboundedInMem<T, BundleT>> {
		: public TransformEvaulator<TransformT_> {

public:
//  using TransformT = UnboundedInMem<T, BundleT>;
	using TransformT = TransformT_;
...
//    const int global_container_threshold = 20;
    ...
    for (auto && t : c->_transforms) {
      ...
      long con = t->container_lists[LIST_ORDERED].size();
...
//  		E("", mbps, mrps);
  ...
  /* NB: starting point of the ts that we generate. note that we should avoid making
   * it epoch (or 0) because the way we calculate start win, see AddAggregatedResult */
  etime current_ts;

  UnboundedInMemEvaluatorBase(int pause_ms = 0)
  	: pause_ms (pause_ms), current_ts(10 * 1000 * 1000) { }
