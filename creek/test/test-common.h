...
#define source_transform_1to3(T) \
	  vector<PCollection *> T##_output = p->applyK(&T, 3); \
	  T##_output[0]->_name = xstr(T##_out0);  \
		T##_output[1]->_name = xstr(T##_out1); \
		T##_output[2]->_name = xstr(T##_out2)
...
#define connect_transform_1to3(S, O0, O1, O2) \
	auto O0##_output = dynamic_cast<PCollection *>(S##_output[0]->apply1(&O0)); \
	O0##_output -> _name = xstr(O0##_out); \
	auto O1##_output = dynamic_cast<PCollection *>(S##_output[1]->apply1(&O1)); \
	O1##_output -> _name = xstr(O1##_out); \
	auto O2##_output = dynamic_cast<PCollection *>(S##_output[2]->apply1(&O2)); \
	O2##_output -> _name = xstr(O2##_out) \
...
#define connect_transform_2to1(T1, T2, S) \
    ...
    xzl_assert(S##_output == S##_output1);  /* same output */ \
		xzl_bug_on_msg(!( \
									(T1.get_side_info() == SIDE_INFO_R && T2.get_side_info() == SIDE_INFO_L) \
							     ||(T1.get_side_info() == SIDE_INFO_L && T2.get_side_info() == SIDE_INFO_R) \
               ), "bug: the two sides must be L/R"); \
		xzl_bug_on_msg(S.get_side_info() != SIDE_INFO_J, "bug: downstream must be J");

#if 0 /* moved to pipeline config */
