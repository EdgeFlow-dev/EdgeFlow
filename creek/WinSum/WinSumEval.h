...
public:
  ...
  void flushState(TransformT* trans, const etime up_wm,
  	 ...
  	 bool purge = true) override
  {
  	 assert(output_bundles);
		...
		etime min_ts_flight = max_date_time;
		{
			if (!conlocked) {
...
//				);
   ...
   */
#if 0
  int fireAsRecordBundle(etime up_wm, TransformT* trans,
      EvaluationBundleContext *c,
...
//    trans->watermark = min(v->min_ts, min(up_wm, trans->min_ts));
  ...
  /* does not do any summation, directly purge the requested window
	  ...
	  above.  */
  void fireAsWindowsBundle(etime watermark, TransformT* trans,
      EvaluationBundleContext *c,
      bool purge = false) {
