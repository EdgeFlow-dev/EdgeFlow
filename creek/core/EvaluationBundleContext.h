...
//#include "Join/Join.h"  /* xzl: no longer maintained */
//#include "Join/JoinSeq.h"
...
#define TASK_QUEUE_EVAL 	0
#define TASK_QUEUE_SOURCE 1
#define TASK_QUEUE_MAX 		2
...
class EvaluationBundleContext {
	...
	public:
	  ...
	  std::function<void(int)> pop(int qid) {
	      std::function<void(int id)> * _f = nullptr;
	      this->q.at(qid).pop(_f);
	  ...
	  auto push(int qid, F && f, Rest&&... rest) ->std::future<decltype(f(0, rest...))> {
...
//	      this->cv.notify_one();
	  ...
	  template<typename F>
	  auto push(int qid, F && f) ->std::future<decltype(f(0))> {
...
//	  TaskQueue<std::function<void(int id)> *> q;  /* task queue */
		...
		/* we use at() to access it which checks bounds */
		std::array<TaskQueue<std::function<void(int id)> *>, TASK_QUEUE_MAX> q;

		/* cf: is_pressure_high_bundle_containers() in UnboundedInMemEvaluatorBase
		 * @cnt: (out) bundle count in the pipeline
		 * */

		static bool is_pressure_high_bundle_containers(EvaluationBundleContext *c, unsigned long *cnt = nullptr) {
			unsigned long total_pending = 0;
			...
			//if (total_pending > 2 * config.bundles_per_epoch) /* threshold */
			if (total_pending > 50) /* threshold */
				return true;
			else
				return false;
		}
		...
		static void * run(void *t) {
			...
			while (true) {
				...
				while (true) {
					...
					/* drain the *eval* task queue (directly pop from the task queue?) */
					while ( (isPop = exec->q.at(TASK_QUEUE_EVAL).pop(_f)) ) {  // if there is anything in the queue
//					if ( (isPop = exec->q.at(TASK_QUEUE_EVAL).pop(_f)) ) {  // if there is anything in the queue
							std::unique_ptr<std::function<void(int id)>> func(_f); // at return, delete the function even if an exception occurred
							try {
								(*_f)(id);
					...
					unsigned long cnt;
					if (is_pressure_high_bundle_containers(c, &cnt)) {
						VV("is_pressure_high_bundle_containers=T, skipping source. pending = %lu", cnt);
						goto container;
					}

					/* drain the source task queue as long as downstream pressure is low.
					 * under normal operation, source tasks come in bursts; we can always
					 * finish them and go after other tasks */
					while ( (isPop = exec->q.at(TASK_QUEUE_SOURCE).pop(_f)) ) {  // if there is anything in the queue
						std::unique_ptr<std::function<void(int id)>> func(_f); // at return, delete the function even if an exception occurred
						try {
							(*_f)(id);
						} catch (const std::exception& e) {
						...
						if (is_pressure_high_bundle_containers(c)) {
							VV("is_pressure_high_bundle_containers=T, leaving source");
							break;
						}
					}
...
container:
	...
	//				while ((bundleptr = c->getOneBundle(&trans))) {
					...
					/* this iteration got nothing from task or containers. sleep. */
					if (!has_work)
						break;
				}
...
//  unordered_set<PTransform *> _transforms;
  ...
  EvaluationBundleContext(int target_wm_delta_secs, unsigned int num_workers)
    : _visitor(this),
      tp(0),
      num_workers(num_workers),
      executor_(this, num_workers),
      target_wm_delta_(target_wm_delta_secs * USEC_PER_SEC)
  {
...
private:
   ...
   * to a set as well. */
	unordered_set<PTransform *> visited_;
    ...
    while (true) {
        ...
        if (!t)
          break;
				...
				if (!visited_.count(t)) { // not visited (if visited, shall we break here?)
...
#if 1
				xzl_bug_on_msg(t->outputs.size() > 3, "TBD");

        if (t->outputs.size() == 0)
          break;

				if (t->outputs.size() == 3) {
					traverseTopology(t->outputs[2]);
...
//  void OnNewUpstreamWatermark(etime wm, PTransform *trans);
  ...
  void PropogateWatermarkSnapshot(PTransform *start, etime const start_wm) {
#if 0
...
//    RefreshTransformWatermark(wm, p);
    ...
    while (l.size()) {
      ...
      etime wm = pair.second;  /* upstream's wm snapshot */
      etime this_wm;
...
//				etime old_wm = p->watermark; /* peek */
				etime old_wm = p->GetWatermarkSafe(); /* peek */
...
//				OnNewUpstreamWatermark(wm, p);
  ...
  void SeedWatermark(etime watermark) {
    assert(source);
...
//    source->watermark = watermark;
   ...
   * bundles in between.
   */
  void UpdateSourceWatermark(etime watermark) {
    xzl_assert(source);
    if (source->GetWatermarkSafe() > watermark) {
			EE("error: source watermark "
						 ETIME_FMT
						 " > new watermark "
						 ETIME_FMT,
				 source->GetWatermarkSafe(),
				 watermark);
			xzl_bug("bug");
    } else {
#if 0  /* debugging */
...
//    source->watermark = watermark;
  ...
  void PropagateSourceWatermarkSnapshot(const etime wm) {
  ...
  etime target_wm_ = etime_max; /* initially, we'll consume any bundles */
  etime target_wm_delta_;
...
public:
  ...
  atomic<long> num_bundles_afterwm;
	atomic<long> num_no_bundles; /* # of times failing to get bundle from the pipeline */

  etime AdvanceTargetWm(etime const & target_wm_delta) {
  	boost::unique_lock<boost::shared_mutex> wlock(target_wm_mutex_);
  	target_wm_ += target_wm_delta;
  	return target_wm_;
  }

  etime AdvanceTargetWm() {
  	boost::unique_lock<boost::shared_mutex> wlock(target_wm_mutex_);
  ...
  void SetTargetWm(etime const & t) {
//  	assert(t > target_wm_ && "must advance monotonically");
   ...
   */
  void OnSinkGetWm(etime const & sink_wm) {
  	etime newwm = sink_wm + target_wm_delta_;
  	SetTargetWm(newwm);
//  	AdvanceTargetWm();
  }
  ...
  void getStatistics() {
  	...
  	printf("%25s%10s%10s%10s%10s%10s\n", "Trans", "MB/s", "Avg",
  			"KRec/s", "Avg", "TotalRec/M");

    for (auto && t : this->_transforms) {
    	if (t->ReportStatistics(&s))
    		printf("%25s%10.2f%10.2f%10.2f%10.2f%10.2f\n", s.name,
    				s.lmbps / (1024 * 1024), s.mbps / (1024 * 1024),
    				s.lmrps / (1024), s.mrps / 1024,
						s.total_rec / (1024 * 1024));
    }

		auto listcnt = get_one_bundle_from_list_counter.load(std::memory_order_relaxed);
		auto acnt = get_bundle_attempt_counter.load(std::memory_order_relaxed);
		auto ocnt = get_bundle_okay_counter.load(std::memory_order_relaxed);

		printf("processed num_bundles_beforewm %ld num_bundles_afterwm %ld num_no_bundles %ld \n"
							 "get_one_bundle_from_list_counter %ld \n"
							 "get_bundle_attempt_counter %ld \n"
							 "get_bundle_okay_counter %ld \n",
					 this->num_bundles_beforewm.load(std::memory_order_relaxed),
					 this->num_bundles_afterwm.load(std::memory_order_relaxed),
					 this->num_no_bundles.load(std::memory_order_relaxed),
					 listcnt,
					 acnt,
					 ocnt
		);

		if ((float) ocnt / acnt < 0.8) {
			EE_WARN("warning: too many failed attempts to GetBundle from container. Cost may be high");
		}
...
//  		t->dump_containers();
  ...
  shared_ptr<BundleBase> getOneBundle(PTransform ** ptrans, int node = -1) {
  	...
  	/* Steal jobs based on the targeted wm */
		for (auto & t : this->_transforms) {
			auto bundleptr = t->getOneBundleOlderThan(target_wm_,
																								&has_pending_punc, node);
			if (bundleptr) {
				*ptrans = t;
...
#if 0
			if ((*it)->get_side_info() == SIDE_INFO_JDD) {
				auto bundleptr = (*it)->getOneBundle_JDD(node);
				if (bundleptr) {
					*ptrans = *it;
					num_bundles_afterwm.fetch_add(1, memory_order_relaxed);
					return bundleptr;
				}
			} else if ((*it)->get_side_info() == SIDE_INFO_JD) {
				auto bundleptr = (*it)->getOneBundle_JD(node);
				if (bundleptr) {
					*ptrans = *it;
					num_bundles_afterwm.fetch_add(1, memory_order_relaxed);
					return bundleptr;
				}
			} else if ((*it)->get_side_info() == SIDE_INFO_J) {
				/* XXX xzl: this is REALLY nasty. try to cast transform into a series of possible types
 				* should have just overriding Base's getOneBundle()
				 *
				 * the control structure is weird but seems no better way
 				* */
				shared_ptr<BundleBase> bundleptr = (*it)->getOneBundle(node);
				#if 0
				Join<pair<long, long>> *jt = dynamic_cast<Join<pair<long, long>> *>(*it);
				if (jt) {
					bundleptr = jt->getOneBundle(node);
				} else {
					auto *jt = dynamic_cast<JoinSeq<creek::kvpair, 0> *>(*it);
					if (jt)
						bundleptr = jt->getOneBundle(node);
					else {
						auto *jt = dynamic_cast<JoinSeq<creek::kvpair, 1> *>(*it);
						if (jt)
							bundleptr = jt->getOneBundle(node);
						else {
							auto *jt = dynamic_cast<JoinSeq<creek::k2v, 0> *>(*it);
							if (jt)
								bundleptr = jt->getOneBundle(node);
							else {
								auto *jt = dynamic_cast<JoinSeq<creek::k2v, 1> *>(*it);
								xzl_bug_on_msg(!jt, "failed to convert JoinSeq to known transform types");
								bundleptr = jt->getOneBundle(node);
							}
						}
					}
...
//  		t->dump_containers();
  	...
  	/* XXX should work on bundles behind the target wm? */
		num_no_bundles.fetch_add(1, memory_order_relaxed);
		return nullptr;
  }
