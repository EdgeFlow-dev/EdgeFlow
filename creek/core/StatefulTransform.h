 ...
 *
 *
 * The evaluator may aggregate its bundle locally, which is not yet submitted to
 * the transform's internal state;
 * Later, after locking the transform, the eval can combine the local aggregation result
...
/* WindowResultT:
 * 	one window's agg result in the transform's internal state. Not exposing to downstream transforms.
 * LocalWindowResultT:
 * 	one window's agg result held by an evaluator. The format, for opt reason, may differ from WindowResultT
 *
 * NB: WindowResultT will be *copied* back/forth in submitting/retrieving
 * states, so it'd better to be pointers.
 */
template <typename TransformT, typename InputT,
		typename WindowResultT = InputT,
...
public:
	...
	using time_duration = etime;

private:

	/* Internal agg results (state) organizes as a seq of windows.
	 *
	 * For this, we tried tbb::concurrent_unordered_map or tbb::concurrent_vector
	 * and abandon them. The reason is that the concurrent growth is limited.
	 * See comments inline. */
	using SavedResultT = vector<WindowResultT>;

	/* take reader lock for aggregation (update a specific window's res) and grow
	 * (add a new window).
	 ...
	 * be careful: aggressive growth can be very slow if ResultT is a sophisticated
	 * d/s. */
//  const long partial_results_grow_ = 4 * 1024 * 1024 / sizeof (WindowResultT);
	const long partial_results_grow_ = 32;

	/* the threshold for GC internal stale items */
#ifdef USE_TZ
	const long partial_results_gc_ = 0; /* xzl: for xplane: gc immediately */
#else
	const long partial_results_gc_ = 0 * partial_results_grow_;	// used to be 10
#endif

	bool is_start_win_assigned(void) {
		return (this->start_win_.duration != 0);
	}

	void clear_start_win(void) {
		/* clean both start & duration */
		this->start_win_.start = 0;
		this->start_win_.duration = 0;
	}

	/* The duration of a window.
	 * Unlike start_win, once assigned, these won't be changed/invalidated.
	 * So no lock needed to read  */
	etime win_duration_ = -1;

	void set_win_duration(time_duration const & d) {
		if (win_duration_ == -1) { /* unassigned */
			win_duration_ = d;
		} else {
			xzl_assert(win_duration_ == d);
		}
	}
...
//
//		long index = (win.start - this->start_win_.start).total_milliseconds()
...
//		return index;
		...
		long index = (win.start - start) / this->win_duration_;

		xzl_assert(index >= 0);

		if (!(win.start == index * this->win_duration_ + start)) {
			EE("bug: start_win.start " ETIME_FMT " win.start " ETIME_FMT
						 " diff" ETIME_FMT " index %ld duration " ETIME_FMT,
				 start,
				 win.start,
				 (win.start - start), index,
				 this->win_duration_);

			cout << "this window:" << to_simple_string(win.window_start()) << endl;
			cout << "win start:" << to_simple_string(this->start_win_.window_start()) << endl;
		}

		xzl_assert(win.start  == index * this->win_duration_ + start);
		return index;
	}

	Window index_to_win(long idx) {
		xzl_assert(idx >= 0);
		return Window(start_win_.start + idx * win_duration_, win_duration_);
	}
	 ...
	 *
	 */
	long wm_to_index(etime const & wm, bool must_align = false) {

		etime span = (wm - this->start_win_.window_start());
		etime & winspan = this->win_duration_;
	 ...
	 */
	etime wm_rounddown(etime const & wm) {
	...
	etime wm_rounddown_epoch(etime const &wm,
													 etime const &duration) {

		etime span = (wm - creek::epoch);
		etime winspan = duration;

		EE("span is " ETIME_FMT, span);
		xzl_assert(span >= 0);
		xzl_assert(winspan && winspan != -1);

		etime offset = (span % winspan);

		return (wm - offset);
	}

public:
	StatefulTransform(string name, int sideinfo)
			: PTransform(name, sideinfo), unflushed_(0) {
		/* so that we know start_win_ is unassigned */
		start_win_.duration = 0;
	}
	 ...
	 *
	 * xzl: @in can't be const because we may ExpandP()
	 */
	void AddAggregatedResult(LocalAggResultT & in) {
		/* reader lock.
		 * see comment for mtx_partial_res. */

		boost::shared_lock<boost::shared_mutex> rlock(this->mtx_partial_results_);
		...
		for (auto & win_res : in) {
			...
			if (!is_start_win_assigned()) {
				...
				rlock.unlock();
				{
					...
					/* Some concurrent writers may have slipped in & set the start
					 * window. check again */
					if (!is_start_win_assigned()) {

						xzl_assert(unflushed_ == 0);

						/* Try to allocate several windows earlier than the current window
						 * we receive.
						 *
						 * This evaluator comes with @win, but a later eval may come w/
						 * even earlier window. Reset the start window (backwards) is a
						 * pain.
						 *
						 ...
						 * of this @win.
						 *
						 * This also helps dealing with the initial a few sliding windows.
						 */

						start_win_ = Window(
								wm_rounddown_epoch(win.window_start() > 10 * 1000 * 1000 ? win.window_start() - 10 * 1000 * 1000 : 0,
																	 win.duration),
								win.duration);
						set_win_duration(win.duration);

						EE("start win unassigned. incoming win %s, set start win %s",
				 ...
				 * There's a chance of GC which cleans up start_win_ again. have to
				 * restart.
				 */
				rlock.lock();
				goto restart;
			...
			if (win.start < this->start_win_.start) {
				EE("bug. win start "
							 ETIME_FMT
							 "< start_win "
							 ...
							 ETIME_FMT
							 "but ignore",
					 win.start, this->start_win_.start);
				return;
			}
...
//			EE("index is %ld", index);
			 ...
			 * seems that elements under construction may be exposed to concurrent
			 * readers --> segfault
			 *
			 * With concurrent vector, we may overgrow (concurrent) but that's okay. */
...
//					EE("try to grow");
					...
					if (winstart == this->start_win_.start
						...
						/* GC didn't happen. but concurrent growth may have happened.
...
//							partial_results_.resize(s + delta, acc);
							...
							for (int k = s; k < s + delta; k++) { /* init each window result explicitly */
								dynamic_cast<TransformT*>(this)->aggregate_init(&partial_results_[k]); /* downcast */
								partial_results_[k].w = index_to_win(k); /* must init win, too */
							}
						}
					} else {
			...
			/* NB -- this has to be MT safe */
			dynamic_cast<TransformT*>(this)->combine(partial_results_[index], res);
	...
	<--+---+---+---+---+---+
	 ...
	 be atomic.
	 In fact, we do not need to care for concurrent flushers. A later punc is only
	 retrieved *after* the prior one is consumed.

	 NB: it may return window w/o no records.

	 */
	etime RetrieveState(AggResultT* out, etime const & wm_ret,
											etime const & wm_flush)
	{
		...
		long flush_index = wm_to_index(wm_flush); /* okay that wm is unaligned to window boundary */
		long max_index = s - 1;
		xzl_bug_on(max_index < 0);
		...
		if (ret_index < flush_index) {
			xzl_bug("bug? although this is supported");
		}

		if (ret_index < unflushed || flush_index < unflushed) {
			xzl_bug("bug? try to access GC'd state");
		}
		...
		I("flush %ld -- %ld, ret %ld -- %ld",
			unflushed, flush_index, unflushed, ret_index);

		k2_measure("retindex_computed");

		for (int i = unflushed; i <= ret_index; i++) {
			/* compute the window for each returned value. */
			Window w(start_win_.window_start() + i * win_duration_,
							 start_win_.duration);
//  		ret[w] = partial_results_[i]; /* copy */
		 ...
		 * Since we return copies in flushing (or smart pointers), GC here is safe.
		 *
	 ...
	 *
	 * as a side effect of retrieval, the 1st delta of the superwindow is flushed.
	 *
	 * Limitation:
	 ...
	 * deltas.
	 */
	etime RetrieveStateSliding(AggResultT* out, etime wm_up, int multi)
	{
		etime t = wm_rounddown(wm_up); /* end of the last retrieved window */

		/* start of the latest sliding window. anything earlier than that won't
		 * show up in later sliding windows. */
		etime flush = t - (multi * this->win_duration_);
		xzl_assert(t > etime(etime_min) + (multi * this->win_duration_)
							 && "etime value underflow.");
	...
	etime RetrieveState(AggResultT* out,
											bool purge = false, etime wm = etime_max, long n = -1)
	{
		auto & ret = *out;
		...
		long cnt;
		if (wm == etime_max)
			cnt = partial_results_.size() - unflushed; /* all items. can be 0. */
		else {
			cnt = (wm - start_win_.window_start())
						/ start_win_.duration - unflushed;
			/* e.g.
		...
		high_index = min(unflushed + cnt - 1,
										 (long)(partial_results_.size() - 1));

		if (high_index == unflushed - 1) {
			/* this can happen when the flush range is empty. e.g. cnt == 0 */
			VV("partial_results_.size() %zu "
						 "unflushed %ld cnt %ld high_index %ld", partial_results_.size(),
				 unflushed, cnt, high_index
			);
			return start_win_.window_start(); /* nothing flushed. unchanged */
		}

		xzl_bug_on(high_index < unflushed - 1);
		...
		I("flush %ld -- %ld", unflushed, high_index);

		for (long i = unflushed; i <= high_index; i++) {
			/* compute the window for each returned value. */
			Window w(start_win_.window_start() + i * start_win_.duration,
							 start_win_.duration);
			//  		ret[w] = partial_results_[i]; /* copy */
			ret.emplace(w, partial_results_[i]);
		}
		...
		if (unflushed == s) {
			 ...
			 * */
			ret_min_ts = start_win_.window_start()
									 + unflushed * start_win_.duration;
		} else {
			ret_min_ts = 0;
			xzl_bug("bug: unflushed too large?");
		}
			...
			rlock.unlock();
			{
				...
				if (unflushed_ > partial_results_gc_) {
					...
					if (unflushed_ == s) { /* empty internal state */
						...
						start_win_.start
								+= (unflushed_ * start_win_.duration);
					}
   ...
   */
  static WindowResultT const & combine(WindowResultT & mine, WindowResultT const & others);
#endif

#if 0 // factored into PTransform
