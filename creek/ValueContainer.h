...
public:
  creek::ptime min_ts = etime_max; // user has to maintain this.
...
public:
  creek::etime min_ts = etime_max; // user has to maintain this.
...
//  	vals.insert(vals.end(), other.vals.begin(), other.vals.end());
  ...
  void add_v(V const & v, creek::etime const & ts) {
    if (vals.size() == 0) {
...
{
	vector<V> vals;
  creek::etime min_ts = etime_max;  /* we may get ride of this */
