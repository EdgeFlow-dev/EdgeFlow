...
#include "xplane/xplane_common.h"
...
batch_t *merge_two_batches(batch_t *inA, batch_t *inB, uint32_t t_idx, idx_t reclen) {
	...
	int32_t *out;
	int32_t sizeA = SIZE_TO_ITEM(inA->size, reclen);
	int32_t sizeB = SIZE_TO_ITEM(inB->size, reclen);
...
//	batch_t *dest = batch_new_after(inB, X_CMD_SORT);
	batch_t *dest = batch_new(X_CMD_SORT, (inA->size + inB->size) * sizeof(simd_t));
...
#ifdef IS_SORTED
	if(!x_is_sorted((int32_t *) dest->start, dest->size / reclen, t_idx))
...
//		XMSG("make L1 chunk [size] : %d", nitems);
		left = batch_new(X_CMD_SORT, ITEMS_PER_L1 * reclen * sizeof(simd_t));
		if(!left)
		    return NULL;
#ifdef TIME_MEASURE
				clock_gettime(CLOCK_MONOTONIC, &t_start);
