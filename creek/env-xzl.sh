...
SERVER=makalu
#SERVER=knl
LOCAL_PATH=/home/xzl/creek
REMOTE_PATH=$SERVER:/home/xzl/creek
...
config-creek() {
	# config debug
	cd ${ROOTDIR}/cmake-build-debug-${ARCH}
	/usr/bin/cmake \
	...
	# config release
	cd ${ROOTDIR}/cmake-build-release-${ARCH}
	/usr/bin/cmake \
...
build-dbg-creek() {
	/usr/bin/cmake \
	--build ${ROOTDIR}/cmake-build-debug-${ARCH} \
	--target all -- \
	-j `nproc`
}
...
build-rls-creek() {
	/usr/bin/cmake \
	--build ${ROOTDIR}/cmake-build-release-${ARCH} \
	--target all -- \
	-j `nproc`
}
...
mkdir -p cmake-build-debug-${ARCH}
mkdir -p cmake-build-release-${ARCH}
