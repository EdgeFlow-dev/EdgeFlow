...
*/
  ...
  struct KVPairContainer{
	...
	//KVPairContainer_radix(etime start)
        KVPairContainer(etime start)
                : start(start), max(start){}
...
// use KVPaireContainer's pointer in map
  struct WindowsKVPairContainer {
	  ...
	  //map<etime, KVPairContainer> containers;
	  map<etime, shared_ptr<KVPairContainer>> containers;
	  ...
	  etime watermark_update(etime const & ts) {
	  ...
	  etime purge_containers(etime cutoff) {
		  ...
		  assert(containers.size());
		  if(containers.empty()){
		  	etime now = boost::posix_time::microsec_clock::local_time();
			return now;
		  }
		  */
		  ...
		  //etime last_container_max = containers.rbegin()->second.max;
		  //etime last_container_max = containers.rbegin()->second->max;
		  etime last_container_max = containers.rbegin()->second->max;
...
//				  std::cout << __FILE__ << __LINE__ << ": containers.erase(it)" << std::endl;
	  ...
	  bool search(etime start, etime end, K const & key,
			  RecordKV * rec) {
...
//hym: split this functio into watermark_update() and flush_state
  ...
  void watermark_update(etime const & ts, int i) {
  ...
  Join(string name, etime window_size)
    : PTransform(name), window_size(window_size) {
		 ...
		 */
		/* xzl: above: why do we init to local time?? */
			left_wm = 0;
			right_wm = 0;
...
public:
  ...
  shared_ptr<BundleBase> getOneBundleOlderThan_left(etime const & t,
...
//		dump_containers("no bundles");
  ...
  shared_ptr<BundleBase> getOneBundleOlderThan_right(etime const & t,
  		bool* has_pending_punc, int node = -1) {
...
//		dump_containers("no bundles");
  ...
  shared_ptr<BundleBase> getOneBundleOlderThan(etime const & t,
