#include "xplane_join.h"
#include "params.h"

extern void sz_show_resource(void);

int32_t join_bykey(x_addr *dest, batch_t *srcA, batch_t *srcB, x_params *params){
	batch_t *dest_batch = batch_new(X_CMD_JOIN);
	idx_t reclen        = params->reclen;
	idx_t keypos        = params->keypos;

	int32_t *dest_data  = dest_batch->start;

	int32_t *srcA_data = srcA->start;
	int32_t *srcB_data = srcB->start;
	int32_t *src_tmp;
	...
	while((void *)srcA_data < srcA->end && (void *)srcB_data < srcB->end) {
		if(srcA_data[keypos] < srcB_data[keypos])
			...
			do{
				matches++;
				srcA_data += reclen;
			} while(srcA_data[keypos] == matched_key && (void *)srcA_data < srcA->end);


			memcpy(dest_data, src_tmp, sizeof(int32_t) * (srcA_data - src_tmp));
			...
			do {
				matches++;
				srcB_data += reclen;
			} while (srcB_data[keypos] == matched_key && (void *)srcB_data < srcB->end);

			memcpy(dest_data, src_tmp, sizeof(int32_t) * (srcB_data - src_tmp));
...
int32_t join_byfilter(x_addr *dest, batch_t *srcA, batch_t *srcB, x_params *params){
	batch_t *dest_batch = batch_new(X_CMD_JOIN);
	idx_t reclen        = params->reclen;
	...
	while((void *)srcA_data < srcA->end){
		if(srcA_data[vpos] > srcB_data[vpos]){
			memcpy(dest_data, srcA_data, sizeof(int32_t) * reclen);
			dest_data += reclen;
		}
		srcA_data += reclen;
	}
...
TEE_Result xfunc_join(uint32_t type, TEE_Param p[TEE_NUM_PARAMS]){
	...
	if (!srcA->size || !srcB->size) {
		...
		for (uint32_t i = 0; i < params->n_outputs; i++) {
			tmp_batch = batch_new(X_CMD_JOIN);
	...
	switch(params->func){
		case join_bykey_t:
			xret = join_bykey(outbuf, srcA, srcB, params);
			break;
		case join_byfilter_t:
			xret = join_byfilter(outbuf, srcA, srcB, params);
			break;
		default:
			break;
	}
